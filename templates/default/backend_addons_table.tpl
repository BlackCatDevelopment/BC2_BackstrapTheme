...
{/if}
{if !$modules && !$zips}
    <div class="alert alert-info">{translate('Nothing to show')}</div>
{else}
{if $current=="catalog"}
...
{/foreach}
{if count($zips)}{foreach $zips z}
            <tr>
                <td class="bs-module-type" data-search="zips">
                    <span class="fa fa-fw fa-file-archive" title="{translate('Zip')}"></span>
                </td>
                <td class="bs-module-name">{$z.filename}</td>
                <td></td>
                <td class="bs-module-desc">
                    {translate('Size')}: {$z.size} | {translate('Date')}: {$z.date}
                </td>
            </tr>
{/foreach}{/if}
        </tbody>
